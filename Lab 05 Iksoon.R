...
Senior <- S.States %>% 
  mutate(POP.1000 = (DP0010015+DP0010016+DP0010017+DP0010018+DP0010019)
         /(DP0010001)*1000) %>% dplyr::select(POP.1000, County=NAMELSAD10)
...
Dataset_2 <- Senior.projected %>% 
  ...
  dplyr::select(County, POP.1000, p_value, Quadrents)
...
# It may take around 30 seconds, and show warning messages like text.
leaflet() %>%
